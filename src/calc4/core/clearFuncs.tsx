...
const cutIfPeriod = (s: string) => {
  const i = s?.indexOf(".");
  if (i !== -1) {
    const tail = s.slice(i);
    return s.slice(0, i) + tail.slice(0, 9);
  }
  return s;
};
...
export const addRes = (display: string[], journal: string[], type: string) => {
  ...
  const res = cutIfPeriod(calc(str));
...
export const fastRes = (display: string[]) => {
  display = addIfNeedCloseScope(display);
  const str = prepareStr(display);
  const res = calc(str.replace(/[-+*/(]\)?$/, ""));
  return prepareRes(cutIfPeriod(res));
};
